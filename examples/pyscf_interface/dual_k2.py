...
class dual_sdp():
    ...
    def __init__(self, nalpha, nbeta, nmo, oei, tei, q2 = False, d2 = False, t2 = False):
        """
        SDP problem
        ...
        # block dimensions (for libsdp solver)
        self.dimensions = []
        ...
        self.dimensions.append(nmo*nmo) # g2abab
        self.dimensions.append(nmo*nmo) # g2baba
        self.dimensions.append(nmo*(nmo-1)//2) # d2aa
        self.dimensions.append(nmo*(nmo-1)//2) # d2bb
        self.dimensions.append(nmo*nmo) # d2ab
        self.dimensions.append(1) # lagrange multplier for the trace of d2
        #self.dimensions.append(1) # lagrange multplier for the trace of d2aa
        #self.dimensions.append(1) # lagrange multplier for the trace of d2bb
        #self.dimensions.append(1) # lagrange multplier for the trace of d2ab
        ...
        #blocks = ['1', 'd1a', 'd1b', 'g2aa', 'g2ab', 'g2ba', 'd2aa', 'd2bb', 'd2ab'] 
        blocks = ['d1a', 'd1b', 'g2aa', 'g2ab', 'g2ba', 'd2aa', 'd2bb', 'd2ab'] 
        ...
        #constraints = ['d2aa', 'd2bb', 'd2ab'] 
        constraints = ['d2']

        # constraint id ... start whereever block ids end
        self.constraint_id = {
        }
        for constraint in constraints:
            self.constraint_id[constraint] = count
            count = count + 1
    
        # number of blocks
        nblocks = len(self.dimensions)

        # reduced hamiltonian (aa)
        K2aa = np.zeros([len(self.bas_aa), len(self.bas_aa)])
        for ij in range (0, len(self.bas_aa)):
            ...
            for kl in range (0, len(self.bas_aa)):
                k = self.bas_aa[kl][0]
                l = self.bas_aa[kl][1]
                K2aa[ij][kl] = tei[i][k][j][l]
                if i == k :
                    K2aa[ij][kl] += oei[j][l] / ( nalpha + nbeta - 1 )
                if i == l :
                    K2aa[ij][kl] -= oei[j][k] / ( nalpha + nbeta - 1 )
                if j == k :
                    K2aa[ij][kl] -= oei[i][l] / ( nalpha + nbeta - 1 )
                if j == l :
                    K2aa[ij][kl] += oei[i][k] / ( nalpha + nbeta - 1 )

        # reduced hamiltonian (ab)
        K2ab = np.zeros([len(self.bas_ab), len(self.bas_ab)])
        for ij in range (0, len(self.bas_ab)):
            ...
            for kl in range (0, len(self.bas_ab)):
                k = self.bas_ab[kl][0]
                l = self.bas_ab[kl][1]
                K2ab[ij][kl] = tei[i][k][j][l]
                if i == k :
                    K2ab[ij][kl] += oei[j][l] / ( nalpha + nbeta - 1 )
                if j == l :
                    K2ab[ij][kl] += oei[i][k] / ( nalpha + nbeta - 1 )

        #import scipy
        #w = scipy.linalg.eigh(K2aa, eigvals_only=True)
        #print(w)
        #w = scipy.linalg.eigh(K2ab, eigvals_only=True)
        #print(w)
        
        # F0  ... the integrals in the primal form ... the constraints in the dual form
        ...
        # the lagrange multiplier for the trace of d2aa
        block_number.append(self.constraint_id['d2'] + 1)
        row.append(1)
        column.append(1)
        value.append((nalpha + nbeta) * (nalpha + nbeta - 1) / 2)

        # the lagrange multiplier for the trace of d2aa
        #block_number.append(self.constraint_id['d2aa'] + 1)
        #row.append(1)
        #column.append(1)
        #value.append(nalpha * (nalpha - 1) / 2)

        ## the lagrange multiplier for the trace of d2bb
        #block_number.append(self.constraint_id['d2bb'] + 1)
        #row.append(1)
        #column.append(1)
        #value.append(nbeta * (nbeta - 1) / 2)

        ## the lagrange multiplier for the trace of d2ab
        #block_number.append(self.constraint_id['d2ab'] + 1)
        #row.append(1)
        #column.append(1)
        #value.append(nalpha * nbeta)
        ...
        for ij in range (0, len(self.bas_aa)):
            ...
            for kl in range (0, len(self.bas_aa)):
                ...
                # i^ j^ l k (D2 aa aa)
                #block_number.append(self.block_id['d2aa'] + 1)
                #row.append(ij + 1)
                #column.append(kl + 1)
                #value.append(1.0)
                ...
                # g2aaaa (two-body part)
                il = self.ibas_ab[i, l]
                kj = self.ibas_ab[k, j]
                block_number.append(self.block_id['g2aa'] + 1)
                row.append(il + 1)
                column.append(kj + 1)
                value.append(-1.0)
                ...
                if i == k :
                    ...
                    # g2aaaa (one-body part)
                    block_number, row, column, value = self.one_body_contribution_12_1(block_number, row, column, value, self.block_id['g2aa'],0, 0, j, l, 1)

                    # g2abab (one-body part)
                    block_number, row, column, value = self.one_body_contribution_12_1(block_number, row, column, value, self.block_id['g2ab'],0, 0, j, l, 1)

                if i == l :

                    # g2aaaa (one-body part)
                    block_number, row, column, value = self.one_body_contribution_12_1(block_number, row, column, value, self.block_id['g2aa'],0, 0, j, k, -1)

                    # g2abab (one-body part)
                    block_number, row, column, value = self.one_body_contribution_12_1(block_number, row, column, value, self.block_id['g2ab'],0, 0, j, k, -1)

                if j == k :

                    # g2aaaa (one-body part)
                    block_number, row, column, value = self.one_body_contribution_12_1(block_number, row, column, value, self.block_id['g2aa'],0, 0, i, l, -1)

                    # g2abab (one-body part)
                    block_number, row, column, value = self.one_body_contribution_12_1(block_number, row, column, value, self.block_id['g2ab'],0, 0, i, l, -1)

                if j == l :

                    # g2aaaa (one-body part)
                    block_number, row, column, value = self.one_body_contribution_12_1(block_number, row, column, value, self.block_id['g2aa'],0, 0, i, k, 1)

                    # g2abab (one-body part)
                    block_number, row, column, value = self.one_body_contribution_12_1(block_number, row, column, value, self.block_id['g2ab'],0, 0, i, k, 1)
                ...
                self.F.append(F)
                #self.b.append(0.5 * tei[i][k][j][l])
                self.b.append(K2aa[ij][kl])
        ...
        for ij in range (0, len(self.bas_aa)):
            ...
            for kl in range (0, len(self.bas_aa)):
                ...
                # i^ j^ l k (D2 bb bb)
                #block_number.append(self.block_id['d2bb'] + 1)
                #row.append(ij + 1)
                #column.append(kl + 1)
                #value.append(1.0)
                ...
                # g2bbbb (two-body part)
                il = self.ibas_ab[i, l]
                kj = self.ibas_ab[k, j]
                block_number.append(self.block_id['g2aa'] + 1)
                row.append(len(self.bas_ab) + il + 1)
                column.append(len(self.bas_ab) + kj + 1)
                value.append(-1.0)
                ...
                ki = self.ibas_ab[k, i]
                block_number.append(self.block_id['g2aa'] + 1)
                row.append(len(self.bas_ab) + jl + 1)
                column.append(len(self.bas_ab) + ki + 1)
                value.append(+1.0)
                ...
                lj = self.ibas_ab[l, j]
                block_number.append(self.block_id['g2aa'] + 1)
                row.append(len(self.bas_ab) + ik + 1)
                column.append(len(self.bas_ab) + lj + 1)
                value.append(+1.0)
                ...
                li = self.ibas_ab[l, i]
                block_number.append(self.block_id['g2aa'] + 1)
                row.append(len(self.bas_ab) + jk + 1)
                column.append(len(self.bas_ab) + li + 1)
                value.append(-1.0)

                if i == k :

                    # g2bbbb (one-body part)
                    block_number, row, column, value = self.one_body_contribution_12_1(block_number, row, column, value, self.block_id['g2aa'],len(self.bas_ab), len(self.bas_ab), j, l, 1)

                    # g2baba (one-body part)
                    block_number, row, column, value = self.one_body_contribution_12_1(block_number, row, column, value, self.block_id['g2ba'],0, 0, j, l, 1)

                if i == l :

                    # g2bbbb (one-body part)
                    block_number, row, column, value = self.one_body_contribution_12_1(block_number, row, column, value, self.block_id['g2aa'],len(self.bas_ab), len(self.bas_ab), j, k, -1)

                    # g2abab (one-body part)
                    block_number, row, column, value = self.one_body_contribution_12_1(block_number, row, column, value, self.block_id['g2ba'],0, 0, j, k, -1)


                if j == k :

                    # g2bbbb (one-body part)
                    block_number, row, column, value = self.one_body_contribution_12_1(block_number, row, column, value, self.block_id['g2aa'],len(self.bas_ab), len(self.bas_ab), i, l, -1)

                    # g2abab (one-body part)
                    block_number, row, column, value = self.one_body_contribution_12_1(block_number, row, column, value, self.block_id['g2ba'],0, 0, i, l, -1)


                if j == l :

                    # g2bbbb (one-body part)
                    block_number, row, column, value = self.one_body_contribution_12_1(block_number, row, column, value, self.block_id['g2aa'],len(self.bas_ab), len(self.bas_ab), i, k, 1)

                    # g2abab (one-body part)
                    block_number, row, column, value = self.one_body_contribution_12_1(block_number, row, column, value, self.block_id['g2ba'],0, 0, i, k, 1)
        ...
        for ij in range (0, len(self.bas_ab)):
            ...
            for kl in range (0, len(self.bas_ab)):
                ...
                # i^ j^ l k (D2 ab ab)
                #block_number.append(self.block_id['d2ab'] + 1)
                #row.append(ij + 1)
                #column.append(kl + 1)
                #value.append(1.0)
                ...
                # G2abab (two-body part)
                il = self.ibas_ab[i, l]
                kj = self.ibas_ab[k, j]
                block_number.append(self.block_id['g2ab'] + 1)
                row.append(il + 1)
                column.append(kj + 1)
                value.append(-1.0)

                # G2baba (two-body part)
                jk = self.ibas_ab[j, k]
                li = self.ibas_ab[l, i]
                block_number.append(self.block_id['g2ba'] + 1)
                row.append(jk + 1)
                column.append(li + 1)
                value.append(-1.0)
    ...
    def one_body_contribution_12_1(self, block_number, row, column, value, block_id, row_offset, col_offset, id1, id2, sg):
        """
        one-body contributions to k2 from g2 (or other blocks)

        :param block_number: list of block numbers for sdpa sparse format
        :param row:          list of row indices for sdpa sparse format
        :param column:       list of column indices for sdpa sparse format
        :param value:        list of values for sdpa sparse format
        :param block_id:     the relevant block of g2 (or other blocks)
        :param row_offset:   the offset for the rows of g2aaaa/aabb/bbaa/bbbb (or other blocks)
        :param col_offset:   the offset for the columns of g2aaaa/aabb/bbaa/bbbb (or other blocks)
        :param id1:          the row entry
        :param id2:          the column entry
        :param sg:           the sign on the term
        """

        for p in range (0, self.nmo):
            my_row = self.ibas_ab[id1, p]
            my_col = self.ibas_ab[id2, p]
            block_number.append(block_id + 1)
            row.append(row_offset + my_row + 1)
            column.append(col_offset + my_col + 1)
            value.append(sg / (self.nalpha + self.nbeta - 1.0))

        return block_number, row, column, value
