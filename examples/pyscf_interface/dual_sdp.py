...
class dual_sdp():
    ...
    def __init__(self, nalpha, nbeta, nmo, oei, tei, q2 = False, d2 = False, t2 = False):
        ...
        self.dimensions.append(1) # lagrange multplier for the trace of d1
        ...
        self.dimensions.append(1) # lagrange multplier for the trace of d2ba

        # only big block of g2 implemented so far 
        assert(not d2)
        assert(not q2)
        assert(not t2)
        ...
        #blocks = ['1', 'ia', 'ib', 'ia^ja', 'ib^jb', 'ia^jb', 'ib^ja'] 
        #blocks = ['ia', 'ib', 'ia^ja', 'ib^jb', 'ia^jb', 'ib^ja'] 
        ...
        constraints = ['d1a', 'd1b', 'd2aa', 'd2bb', 'd2ab', 'd2ba'] 
        ...
        # the lagrange multiplier for the trace of d1a
        block_number.append(self.constraint_id['d1a'] + 1)
        row.append(1)
        column.append(1)
        value.append(nalpha)

        # the lagrange multiplier for the trace of d1a
        block_number.append(self.constraint_id['d1b'] + 1)
        row.append(1)
        column.append(1)
        value.append(nbeta)
        ...
        # the lagrange multiplier for the trace of d2ba
        block_number.append(self.constraint_id['d2ba'] + 1)
        row.append(1)
        column.append(1)
        value.append(nalpha * nbeta)
        ...
        for i in range (0, nmo):
            for j in range (0, nmo):
                ...
                # i^(alpha), j(alpha)
                block_number.append(self.block_id['d1a'] + 1)
                row.append(i + 1)
                column.append(j + 1)
                value.append(1.0)
                ...
                self.F.append(F)
                self.b.append(oei[i][j] + dum[i][j])

        t = oei + dum
        import scipy
        w = scipy.linalg.eigh(t, eigvals_only=True)
        print(w)
        #exit()

        #return

        # one-electron integrals (beta)
        for i in range (0, nmo):
            for j in range (0, nmo):
                block_number=[]
                row=[]
                ...
                # i^(beta), j(beta)
                block_number.append(self.block_id['d1b'] + 1)
                row.append(i + 1)
                column.append(j + 1)
                value.append(1.0)
                ...
                #    column.append(len(self.bas_ab) + jk + 1)
                #    value.append(1.0)
